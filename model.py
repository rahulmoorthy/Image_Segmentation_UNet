...
class UNet(nn.Module):
    def __init__(self, n_classes):
        ...
        self.u5 = nn.Conv2d(64, 2, kernel_size=1)
...
class downStep(nn.Module):
    def __init__(self, inC, outC, max_pool=True):
        ...
        self.conv1 = nn.Conv2d(inC, outC, kernel_size=3, stride=1)
        #nn.ReLU(inplace=True)
        self.batchnorm1 = nn.BatchNorm2d(outC)
        self.conv2 = nn.Conv2d(outC, outC, kernel_size=3, stride=1)
    ...
    def forward(self, x):
        ...
        x = self.batchnorm1((F.relu(self.conv1(x)))) #self.batchnorm1
        x = self.batchnorm2((F.relu(self.conv2(x)))) #self.batchnorm2
...
class upStep(nn.Module):
    def __init__(self, inC, outC, withReLU=True):
        ...
        self.conv2 = nn.Conv2d(inC, outC, kernel_size=3)
        self.batchnorm1 = nn.BatchNorm2d(outC)
        self.conv3 = nn.Conv2d(outC, outC, kernel_size=3)
        self.batchnorm2 = nn.BatchNorm2d(outC)
        
    def forward(self, x, x_down):
        # todo
        #target_up = self.x
        #target_down = self.x_down
